<?php
    ...
    if ( ! class_exists( 'ReduxFramework' ) ) {
        ...
        class ReduxFramework {
            ...
            public static $_version = '3.6.0.7';
            public static $_dir;
            public static $_url;
            ...
            public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
                ...
                if ( ! empty ( $this->args['opt_name'] ) ) {
					...
					add_action( 'vc_build_admin_page', array($this, 'dfd_vc_remove_woocommerce'), 11 );

					add_action( 'vc_load_shortcode', array($this, 'dfd_vc_remove_woocommerce'), 11 );
					
					add_action('init', array($this, 'remove_extra_vc_elements'));
                 ...
                 * @param  object $this ReduxFramework
                 */
				add_action("switch_theme", array($this, 'generate_options_css'), 10, 3 );
				add_action("redux/options/{$this->args['opt_name']}/saved", array($this, 'generate_options_css'), 10, 3 );
                do_action( 'redux/loaded', $this );
            }

            // __construct()
			
			function enqueue_compiler() {
				require_once locate_template('/inc/lib/CompileLess/CompileLess.php');
				new CompileLess();
			}

			public static function generate_options_css($newdata = '', $old_values = '') {
			...
			function remove_extra_vc_elements() {
				// Removing shortcodes
				if ( function_exists( 'vc_remove_element' ) ) {
					
					if(!isset($this->options['enable_default_modules']) || strcmp($this->options['enable_default_modules'], '1') !== 0) {
						/* Default modules */
			...
			function dfd_vc_remove_woocommerce() {
				if ((!isset($this->options['enable_default_modules']) || strcmp($this->options['enable_default_modules'], '1') !== 0) && is_plugin_active( 'woocommerce/woocommerce.php' ) ) {
	...
	//				vc_remove_element( 'top_rated_products' );
			...
			/**
             * Converts options from collector on save
             *
             * @since   3.5.4.3
             * @return  converted option
             */
			private function convert($options) {
				if(!isset($options['options_values']) || $options['options_values'] == '') return $options;
				
				$result = array();
				
				$this->transients['options_values'] = $options['options_values'];
				
				$new_options = explode('~||~', $options['options_values']);
				
				foreach($new_options as $opt) {
					if($opt != '') {
						
						$opt = explode('~', $opt);
						$start = strlen($this->args['opt_name']) + 1;
						$opt[0] = substr($opt[0], $start, -1);
						if(substr_count($opt[0], '][') == 0) {
							$result[$opt[0]] = $opt[1];
						} else {
							$opt[0] = explode('][', $opt[0]);
							...
							if(isset($opt[0][1])) {
								if(isset($opt[0][2])) {
									$result[$opt[0][0]][$opt[0][1]][$opt[0][2]] = $opt[1];
								} else {
									$result[$opt[0][0]][$opt[0][1]] = $opt[1];
								}
							} else {
								$result[$opt[0][0]] = $opt[1];
							}
						}
            ...
            public function _validate_options( $plugin_options ) {
                ...
                //              exit();
				if(isset($plugin_options['options_values'])) {
					$plugin_options = $this->convert($plugin_options);
				}
                                ...
                                if ( class_exists( $validate ) ) {
                                    ...
                                    if ( isset ( $plugin_options[ $field['id'] ] ) && is_array( $plugin_options[ $field['id'] ] ) && ! empty ( $plugin_options[ $field['id'] ] ) ) {
                                        foreach ( $plugin_options[ $field['id'] ] as $key => $value ) {
                                            ...
                                            if ( isset($validation->value) && ! empty ( $validation->value ) || $validation->value == '0' ) {
            ...
            public function _can_output_css( $field ) {
                ...
                if ( ! empty ( $field['required'] ) ) {
                    if ( isset ( $field['required'][0] ) ) {
                        if ( ! is_array( $field['required'][0] ) && count( $field['required'] ) == 3 && isset($GLOBALS[ $this->args['global_variable'] ][ $field['required'][0] ]) ) {
