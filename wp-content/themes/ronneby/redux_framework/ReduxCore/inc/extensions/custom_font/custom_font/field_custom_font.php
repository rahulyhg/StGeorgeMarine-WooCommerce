...
//			print_r($def_values);
			...
			$defaults = array (
					...
					'content_title' => __('Font', 'redux-framework')
			);

			$this->field = wp_parse_args($this->field, $defaults);
			echo '<div class="redux-slides-accordion2" data-new-content-title="' . esc_attr(sprintf(__('Adding new %s', 'redux-framework'), $this->field['content_title'])) . '">';
...
font-family: "' . $f_name . '";
			...
			foreach ($slides as $slide) {
				...
				$preview_text = $slide["attachment_id"] ? "preview <strong>preview</strong> <em>preview</em> <strong><em>preview</em></strong>" : "";
				...
				echo '<div class="screenshot' . $hide . '" style="
...
">';
				...
				echo '</a>';
				echo '<span id="preview_' . $font_face . '" style="
    vertical-align: middle;
    display: table-cell;
    padding-left: 22px;
    font-size: 25px;
">' . $preview_text . '</span>';
				...
				echo '<span class="button media_upload_font_button" id="add_' . $x . '">' . __('Upload', 'redux-framework') . '</span>';
			...
			echo '</div><a href="javascript:void(0);" class="button redux-slides-add button-primary" rel-id="' . $this->field['id'] . '-ul" rel-name="' . $this->field['name'] . '[title][]' . $this->field['name_suffix'] . '">' . sprintf(__('Add %s', 'redux-framework'), $this->field['content_title']) . '</a><br/><br/>';
		}
