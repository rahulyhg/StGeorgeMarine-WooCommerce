			...
			<div class="seven columns">
							...
							if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
								...
								<tr class="<?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
									...
									<td class="product-name">
										<?php
											...
											if ( $_product->backorders_require_notification() && $_product->is_on_backorder( $cart_item['quantity'] ) )
												echo '<p class="backorder_notification">' . __( 'Available on backorder', 'dfd' ) . '</p>';
										?>
									</td>
									...
									<td class="product-remove">
										<?php
											echo apply_filters( 'woocommerce_cart_item_remove_link', sprintf( '<a href="%s" class="remove" title="%s">×</a>', esc_url( WC()->cart->get_remove_url( $cart_item_key ) ), __( 'Remove this item', 'dfd' ) ), $cart_item_key );
										?>
									</td>
								</tr>
								<?php
				...
				<div class="coupon">
					<?php if ( WC()->cart->coupons_enabled() ) { ?>
						...
						<div class="dfd-coupon-heading"><?php _e( 'Coupon discount', 'dfd' ); ?>:</div>
						<?php /* <div class="subtitle"><?php _e( 'Sign up to get updates & discounts!', 'dfd' ); ?>:</div> */ ?>
						<div class="dfd-coupon-wrap">
							<input type="text" name="coupon_code" class="input-text" id="coupon_code" value="" placeholder="<?php _e( 'Coupon code here', 'dfd' ); ?>" />
							<span><input type="submit" class="button" name="apply_coupon" value="<?php _e( 'Apply Coupon', 'dfd' ); ?>" /></span>
						</div>
