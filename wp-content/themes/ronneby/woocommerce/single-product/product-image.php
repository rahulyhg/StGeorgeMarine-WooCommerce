...
$woo_image_section_class .= dfd_num_to_string_full($columns_var);

$woo_image_section_class .= (isset($dfd_ronneby['woo_single_thumb_position']) && !empty($dfd_ronneby['woo_single_thumb_position'])) ? ' '.$dfd_ronneby['woo_single_thumb_position'] : '';
?>

<div class="images columns <?php echo esc_attr($woo_image_section_class) ?>">
	<div class="single-product-image">
		...
		if ( has_post_thumbnail() ) {
			...
			echo apply_filters( 'woocommerce_single_product_image_html', sprintf( '<div itemprop="image"  id="'. esc_attr($unique_id) .'" class="woocommerce-main-image %s">%s <a href="'.esc_url($image_link).'" class="prettyPhoto" data-rel="prettyPhoto[woo_single_gal]"><img src="%s" alt="" class="dfd-large-image" /></a></div>', $image_title, $image, $image_link ), $post->ID );
			?>
